    ...
    while True:
        try:
            ...
            for thread in threads:
                ...
                # Kullanıcı adını al
                try:
                    sender_info = cl.user_info(msg.user_id)
                    sender = sender_info.username
                except Exception:
                    sender = str(msg.user_id)

                # Gruptaki kullanıcı listesi (ship/kim için)
                try:
                ...
                if is_group:
                    ...
                    if mention.lower() in text.lower():
                        ...
                        if clean:
                            reply = ask_ai(clean, thread_id=tid)
                        else:
                            reply = "Ne yapmamı istiyorsun? 😊 !yardim yaz."
